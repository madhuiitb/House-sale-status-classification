...
rfc_range=list(range(4,20))                       # Its hiper-parameter to explore different hiper-parameter values what will be the accuracy
rfc_accuracy_scores=[]
rfc_maxim=0										  # To get maximum value of accuracy when used different parameters 
rfc_index=0										  # To get maximum index of hiper-parameter 
for i in rfc_range:								  # For loop runnig to change different hiper parameter values
	rfc=RandomForestClassifier(n_estimators=i) # Its a Classifier function with hyper-parameter which has change of hiper-parameter value
	accuracy_scores=cross_val_score(rfc,x_train,y_train,cv=9,scoring='accuracy').mean() # Calculating the accuracy of the RFC with change of hiper-parameter
	if(accuracy_scores>rfc_maxim): 					# ................... 
		rfc_maxim=accuracy_scores 					# To get maximum index of hiper-parameter 
		rfc_index=i 								# ...................
...
print(cross_val_score(rfc,x_train,y_train,cv=9,scoring='accuracy').mean()) # Printing the accuracy of the RFC with higher value of hiper-parameter
...
svc_range=list(range(1,5))							# Its hiper-parameter to explore different hiper-parameter values what will be the accuracy
svc_accuracy_scores=[]
svc_maxim=0											# To get maximum value of accuracy when used different parameters 
svc_index=0											# To get maximum index of hiper-parameter
for i in svc_range:									# For loop runnig to change different hiper parameter values
	#for j in svc_range2:
	svc=svm.SVC(kernel='linear',C=i*20,gamma=0.02) 	# Its a Classifier function with hyper-parameter which has change of hiper-parameter value
	accuracy_scores=cross_val_score(svc,x_train,y_train,cv=9,scoring='accuracy').mean()# Calculating the accuracy of the SVM with change of hiper-parameter
	if(accuracy_scores>svc_maxim):					# ................... 
		svc_maxim=accuracy_scores 					# To get maximum index of hiper-parameter 
		svc_index=(i-1)*20							# ................... 
...
x.close()											 # Closing the x file to avoid garbage  data flow / any other error flow
print(cross_val_score(svc,x_train,y_train,cv=9,scoring='accuracy').mean()) # Printing the accuracy of the SVM with higher value of hiper-parameter
